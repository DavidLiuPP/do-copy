...
DEFAULT_VRP_ASSUMPTIONS = {
    'SKIP_NODE_PENALTY': int(SKIP_NODE_PENALTY),
    'SKIP_NODE_PENALTY_FOR_OPTIONAL_MOVE': int(SKIP_NODE_PENALTY / 2),
    'ADDITIONAL_PENALTY_FOR_ASSIGNED_MOVE': int(SKIP_NODE_PENALTY),
    'CHASSIS_CHANGE_PENALTY': int(SKIP_NODE_PENALTY / 8),
    'NON_DUAL_MOVE_PENALTY': int(SKIP_NODE_PENALTY / 8),
    'VEHICLE_USE_PENALTY': int(SKIP_NODE_PENALTY / 40),

    'PENALTY_FOR_EARLY_INVOCATION': 1,
    'PENALTY_FOR_EMPTY_MILES': 10,
    'PREFERRED_MAX_EMPTY_MILES': 10, # soft upper bound for empty miles
    'MAX_EMPTY_MILES': 30, # hard upper bound for empty miles

    # all times are in minutes
    'MAX_MINUTES': 1440,
    'PREFERRED_START_TIME': int(6 * 60),
    'BUFFER_TIME_FOR_NEXT_NODE': 10,
    'TIME_TO_SWITCH_CHASSIS': 10,
    'MAX_WAITING_TIME_BETWEEN_NODES': 60,
}
