...
async def add_recommended_returns(user_payload: Dict[str, Any], plan_date: str, loads: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    try:
        ...
        if not placeholder_return_location:
            ...
            try:
                ...
                logger.error(f"Failed to load standard ports: {str(e)}")
                return loads
            ...
            try:
                ...
                logger.error(f"Failed to load SSL data: {str(e)}")
                return loads
            ...
            try:
                ...
                logger.error(f"Failed to get recommended returns: {str(e)}")
                return loads
            ...
            try:
                ...
                logger.error(f"Failed to get customer records: {str(e)}")
                return loads
        ...
        for load in loads:
            try:
                ...
                logger.error(f"Error processing load {load.get('reference_number', '')}: {str(e)}")
                mapped_loads.append(load)  # Add original load if processing fails
                continue

        return mapped_loads

    except Exception as e:
        logger.error(f"Failed to add recommended returns: {str(e)}")
        return loads  # Return original loads if function fails
