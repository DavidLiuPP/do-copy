...
async def map_actionable_moves(
    ...
    try:
            ...
            try:
                ...
                if not is_valid_move:
                    ...
                    if is_valid_move:
                        ...
                        # If previous_reason includes '_APPT_CONSTRAINT_FAILED', skip
                        if '_APPT_CONSTRAINT_FAILED' in previous_reason:
                            removed_moves.append(load_copy.get('reference_number', ''))
                        else:
                            invalid_moves.append({
                                'reference_number': load_copy.get('reference_number', ''),
                                'reason': reason
                ...
                invalid_moves.append({
                    'reference_number': load_copy.get('reference_number', ''),
                    'reason': 'ERROR_IN_MAP_ACTIONABLE_MOVES'
                })
                continue
        ...
        return actionable_moves, invalid_moves, removed_moves
    except Exception as e:
        logger.error(e)
...
async def map_loads_for_optimizer(
    ...
    try:
            ...
            try:
                ...
                for move_index, actionable_move in enumerate(moves):
                    ...
                    if is_manually_planned:
                        ...
                        active_event_dates = [
                            datetime.fromisoformat(event.get('departed')) if event.get('departed') else datetime.fromisoformat(event.get('arrived'))
                            for event in actionable_move
                            if event.get('arrived') and not event.get('isVoidOut')
                        ]
                        active_event_date = max(active_event_dates) if active_event_dates else None
                        driver_working_date = active_event_date if active_event_date else load_assigned_date
                ...
                logger.error(f"Error mapping loads for optimizer: {str(e)}")

            if not is_move_included:
                if load.get('error_reason') in ['INVALID_DRIVER']:
                    removed_moves.append(load.get('reference_number', ''))
                else:
                    invalid_moves.append({
                        'reference_number': load.get('reference_number', ''),
        ...
        mapped_actionable_moves, _invalid_moves, _removed_moves = await map_actionable_moves(
            user_payload=user_payload,
        ...
        removed_moves.extend(_removed_moves)

        # check if any of the moves are part of combined move
        if len(loads_with_combined_trips) > 0:
            combined_trips_ref_numbers = [load.get('reference_number', '') for load in loads_with_combined_trips if load.get('reference_number', '')]
            ...
            # Trip numbers set as reference numbers in actionable moves so need to remove move from map_loads for avoid to show in unassigned moves
            removed_moves.extend(combined_trips_ref_numbers)
        ...
        return mapped_actionable_moves, invalid_moves,  removed_moves
